...
class WorkOrderDiagnosticTest {
    ...
    @Test
    fun `analyze WORKORDER PDF OCR extraction`() {
        val pdfPath = "src/test/resources/0101431_01401825_0001_WORKORDER.pdf"
        val file = File(pdfPath)
        val document = Loader.loadPDF(file)

        try {
            println("\n" + "=".repeat(80))
            println("WORKORDER PDF - Zone Extraction Analysis")
            println("=".repeat(80))
            ...
            println("  7-8 digit numbers found: ${sevenEightDigits.map { "${it.text} at (${it.x}, ${it.y})" }}")
            
            val fourDigitsStartingWith0 = ocrWords.filter { it.text.matches(Regex("0\\d{3}")) }
            println("  4-digit numbers starting with 0: ${fourDigitsStartingWith0.map { "${it.text} at (${it.x}, ${it.y})" }}")
